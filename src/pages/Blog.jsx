...
const Blog = () => {
  ...
  const { userId, universityId, userType, userName, BASE_URL } =
  ...
  const handleAddBlog = async () => {
    if (newBlog.title && newBlog.content) {
      ...
      const companySpecificName_TAG =
        newBlog.companyTags.length === 1 ? newBlog.companyTags[0] : "";
  ...
  const handleTagRemove = (tag) => {
    setNewBlog((prev) => ({
      ...prev,
      companyTags: prev.companyTags.filter(
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 py-10 px-4 md:px-6">
      {loading && <Loader />}
      {/* Floating + Button */}
      <button
        onClick={() => setIsModalOpen(true)}
        className="fixed bottom-6 right-6 bg-blue-600 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full text-base sm:text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all transform hover:scale-110"
      >
        +
      </button>
